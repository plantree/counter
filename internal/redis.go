...
func (db *DB) RefreshExpire(key string) error {
	return db.redisClient.Expire(ctx, key, config.REDIS_KEY_TTL).Err()
}

func (db *DB) Set(key string, value interface{}) error {
	// refresh expire automatically
	_, err := db.redisClient.Set(ctx, key, value, config.REDIS_KEY_TTL).Result()
	if err != nil {
...
func (db *DB) BatchGet(keys ...string) ([]RedisResult, error) {
	...
	for _, key := range keys {
		...
		pipe.Expire(ctx, key, config.REDIS_KEY_TTL)
